...
export default class CreateRoleGroup extends Command {
    ...
    public exec(message: Message, {name}: {name: string}): Promise<Message> {
        ...
        if (roleGroups.some(rg => rg.groupName.caseCompare(name.replace(' ', '-')))) return message.channel.send('There is an existing group with this name.')
            ...
            roleGroups.push({ groupName: name.replace(' ', '-'), roles: [], singleLock: false, rolesRequired: [], ignoreRoles: [], messages: [] })
