...
export default class MaxMentions extends Command {
    ...
    public exec(message: Message, {threshold}: {threshold: number}): Promise<Message> {
        const currThreshold = this.client.settings.get(message.guild, 'auto-mod.maxMentions', 0)
        ...
        ;(threshold !== 0 ?
        () => {
            this.client.settings.set(message.guild, 'auto-mod.maxMentions', threshold)
            return message.util!.send('New max mentions threshold has been set successfully.')
        } :
        () => {
            this.client.settings.delete(message.guild, 'auto-mod.maxMentions')
