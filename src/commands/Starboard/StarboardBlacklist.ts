    ...
    *args() {
    ...
    public exec(message: Message, {type, target}): Promise<Message> {
        ...
        if (type === 'user' && target instanceof GuildMember) {
            ...
            if (sbBlacklist.includes(target.user.id)) return message.channel.send('This user is already blacklisted!')
    
            sbBlacklist.push(target.user.id)
    
            this.client.settings.set(message.guild, 'starboard.user-blacklist', sbBlacklist)
            return message.channel.send(`${target} has successfully been blacklisted from the starboard.`)
        }
            ...
            if (sbBlacklist.includes(target.id)) return message.channel.send('This channel is already blacklisted from being noticed.')

            sbBlacklist.push(target.id)

            this.client.settings.set(message.guild, 'starboard.channel-blacklist', sbBlacklist)
            return message.channel.send(`${target} has now been blacklisted from stars being detected.`)
        }
    }
}
